...
function Paginator({ pageIdx, pageNum, clickPage }: PagenatorType) {
  const [pageIdxOffset, setPageIdxOffset] = useState(
    Math.floor((pageIdx - 1) / PAGELISTNUM) * PAGELISTNUM + 1,
  );
  ...
  const makePaginator = () => {
    ...
    for (let i = 0; i < PAGELISTNUM; i++) {
      ...
      pageList.push(
        <li
          key={i + pageIdxOffset}
          className={pageIdx === i + pageIdxOffset ? 'current' : ''}
          onClick={() => clickPage(i + pageIdxOffset)}
        >
          {i + pageIdxOffset}
        </li>,
      );
