...
function FindIdPw({ cancel }: FindIdPwProps) {
  ...
  const idSelectClass = `popup-selector ${isFindId ? 'selected' : ''}`;
  const pwSelectClass = `popup-selector ${isFindId ? '' : 'selected'}`;
    ...
    <div className="popup-wrapper">
      <div className="popup-box">
        ...
        <div className="popup-selector-wrapper">
          <div className={idSelectClass} onClick={() => setIsFindId(true)}>
            아이디 찾기
          </div>
          <div className={pwSelectClass} onClick={() => setIsFindId(false)}>
            패스워드 찾기
          </div>
        </div>
        <div className="popup-contents">{makeForm()}</div>
        <div>{makeMessage()}</div>
        <div className="popup-action">
          {findState === FindStateEnum.IDFOUND ||
          findState === FindStateEnum.PWFOUND ? (
            ...
            <>
              <button
                className="btn-ok"
                disabled={findState === FindStateEnum.LOADING}
                onClick={submit}
              >
                OK
              </button>
