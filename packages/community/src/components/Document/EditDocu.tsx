...
function EditDocu({
  ...
  const makeFileList = () => {
    if (
      editingDocData.attachedFiles &&
      editingDocData.attachedFiles.length > 0
    ) {
      return (
  ...
  return (
    <div className="writepost-wrapper">
      ...
      <div className="btn-wrapper">
        <button className="enif-btn-common enif-btn-cancel" onClick={cancel}>
          {' '}
          취소{' '}
        </button>
        <button className="enif-btn-common enif-btn-ok" onClick={confirm}>
          {' '}
          확인{' '}
        </button>
      </div>
