...
class DocuMenu extends React.Component {
  ...
  createGeneration = () => {
    ...
    for (let i = currentGen; i > 0; i--) {
      genList.push(
        <li key={i} onClick={() => this.props.retrieveDocumentsByGeneration(i)}>
          {i}대
        </li>,
      );
    }
  ...
  render() {
    return (
      <div className="documenu-wrapper">
        <div className="doc-gen-list-handler" onClick={this.leftHandler}>
          &lt;&lt;
        </div>
        ...
        <div className="doc-gen-list-handler" onClick={this.rightHandler}>
          &gt;&gt;
        </div>
      </div>
    );
