...
router.get('/', verifyTokenMiddleware, async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, error: 'RETRIEVE EQUIPMENT FAIL', code: 1 });
  }
});
...
router.get('/search', verifyTokenMiddleware, async (req, res) => {
  ...
  try {
    const { rows, count } = await searchEquipmentList(category_id, status, keyword);
    res.json({ equipCount: count, equipInfo: mapEquipmentRows(rows) });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, error: 'SEARCH EQUIPMENT FAIL', code: 1 });
  }
});
...
router.post('/', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  if (decodedToken.grade > EQUIP_ADMIN_GRADE) {
    return res.status(403).json({ success: false, error: 'PERMISSION DENIED', code: 1 });
  }
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, error: 'CREATE EQUIPMENT FAIL', code: 1 });
  }
});
...
router.patch('/', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  const { id, category_id, name, nickname, description, location, maker, status, img_path } =
    req.body;
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, error: 'UPDATE EQUIPMENT FAIL', code: 1 });
  }
});
...
router.delete('/:id', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, error: 'DELETE EQUIPMENT FAIL', code: 1 });
  }
});
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (equipment.length > 0) {
      return res.status(400).json({ success: false, error: 'EQUIPMENT EXISTS IN CATEGORY' });
    }
    try {
      ...
      console.error(err);
      res.status(500).json({ success: false, error: 'DELETE EQUIPMENT CATEGORY FAIL', code: 1 });
    }
  },
);
...
router.post('/rent', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  res.json({
    successEquipmentIds,
    failedEquipmentIds: equipmentIds.filter((id: number) => !successEquipmentIds.includes(id)),
  });
});
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const result = await returnEquipment(decodedToken._id, parseInt(rentId), photo_path);
      res.json(result);
    } catch (err) {
      console.error(err);
      res.status(500).json({ success: false, error: 'RETURN EQUIPMENT FAIL', code: 1 });
    }
  },
);
...
router.get('/rent/records', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  const dateFields: { key: string; value: string | undefined }[] = [
    { key: 'date_from_start', value: query.date_from_start as string | undefined },
    { key: 'date_to_start', value: query.date_to_start as string | undefined },
    { key: 'date_from_return', value: query.date_from_return as string | undefined },
    { key: 'date_to_return', value: query.date_to_return as string | undefined },
  ];
  for (const { key, value } of dateFields) {
    if (value !== undefined) {
      ...
      if (isNaN(parsed.getTime())) {
        return res
          .status(400)
          .json({ success: false, error: `INVALID DATE FORMAT: ${key}`, code: 1 });
      }
    }
  }
  const filters = {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ success: false, error: 'RETRIEVE ALL RENT RECORDS FAIL', code: 1 });
  }
});
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (isNaN(parsedRentId)) {
      return res.status(400).json({ success: false, error: 'INVALID RENT ID', code: 1 });
    }
    ...
    if (!penalty_status || !allowedStatuses.includes(penalty_status)) {
      return res.status(400).json({ success: false, error: 'INVALID PENALTY STATUS', code: 1 });
    }
    try {
...
router.get('/:id/rents', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const rentList = await retrieveRentListByEquipmentId(parseInt(id), ROWNUM, offset);
    res.json(rentList);
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, error: 'RETRIEVE RENT LIST FAIL', code: 1 });
  }
});
