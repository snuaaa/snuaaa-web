...
import { retrievePhoto, updatePhoto, deletePhoto, retrieveNextPhoto, retrievePrevPhoto, retrievePrevAlbumPhoto, retrieveNextAlbumPhoto, createPhoto, migratePhotos, retrievePhotosWithFilter } from '../controllers/photo.controller';
...
import { retrieveTagsByContent, createContentTag, deleteContentTag, updateContentTag } from '../controllers/contentTag.controller';
...
router.post('/', verifyTokenMiddleware, async (req, res) => {
    ...
    try {
        const contentIdList = await Promise.all(list.map(async (photo: any) => {
            const contentId = await createPhoto({ ...photo, author_id: decodedToken._id, board_id: boardId, album_id: albumId });
