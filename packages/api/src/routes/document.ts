...
router.get('/', verifyTokenMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const docCount = await retrieveDocumentCount(req.query.category, req.query.generation);
...
router.get('/:doc_id', verifyTokenMiddleware, async (req: AuthenticatedRequest, res, next) => {
